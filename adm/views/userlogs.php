...
<input type="button" onclick="Boxy.load('?action=deldata&username=<?php echo $username ?>&fdate=<?php echo $fdate ?>&tdate=<?php echo $tdate ?>',{title:'清除登录日志'})"  class="btn1" value="清除日志" />
...
<td><a class="line blue" href="http://www.baidu.com/s?wd=<?php echo $row['logip'] ?>" target="_blank" title="点击查看登录IP来源"><?php echo $row['logip'] ?></a></td>
<td><?php echo $row['logtime'] ?></td>
<td>
<a href="<?php echo "?action=del&id=".$row['id'] ?>" onclick="if(!confirm('是否要执行 删除 操作？'))return false"><img src="/static/adm/images/ico_del.png"></a>
</td>
</tr>
<?php endforeach;?>
