...
<option value="1" <?php if($state==1) echo 'selected'; ?>>暂停</option>
</select>&nbsp;
...
<td><?php echo $row['is_state'] ? '<span class="red">暂停</span>' : '<span class="green">正常</span>' ?></td>
...
function delData(id){
	if(confirm('是否要要执行此操作？')){
		$.get('msgtpl.php',{action:'del',id:id},function(data){
			if(data=='ok'){
				$('#r_'+id).fadeOut();
			} else {
				alert('删除失败！');
